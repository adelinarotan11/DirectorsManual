...
using DirectorsManual.Web;
using Microsoft.Owin;

namespace DirectorsManual.Web.Controllers
{
	[Authorize]//чтобы получить доступ этому конт, надо быть авторизованым
	public class AccountController : Controller
	{
		...
		public ActionResult Login(string returnUrl)
		{
			...
			return View();//представляет представление Login.cshtml----> затем переход к ассинхр методу Login 
		}
		...
		public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
		{
			if (ModelState.IsValid)
			{
				...
				//}
				var user = await UserManager.FindAsync(model.Email, model.Password);//ищем юзера с заданным логином и паролем
				if (user != null)//если найден
				{
					if (user.EmailConfirmed == true)//проверка подтвержден мейл или нет
					{
			...
			return View(model);
			////отвечает за представление пользователя внутри системы
			//         if (!ModelState.IsValid)//если неверные данные при входе,то возвращается эта же страница с подствеченными полями ввода
			//         {
			//             return View(model);
			//         }
			...
			//         var result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.RememberMe, shouldLockout: false);//сравнивание данных при входе и при регистрации
			//         switch (result)
			//         {
			//             case SignInStatus.Success://успешный вход
			//                 return RedirectToLocal(returnUrl);
